...
#'@author:Zilan Wen
#'
#'The script for Rtudio Exercise 2-Analysis
#'read 'learning2014.txt' from url
...
#'and 56 males. 7 variables include age, attitude, deep combined with deep questions, stra wiht strategiic questions, surf with surface questions, points.
#'Most subjects are at the age of 20-30. The points of attitude and deep questions  for male subjects are higher than that of female. But the points of 
#'strategic and surface quenstions for female subjects are higher than that of males. However there is little difference of the overall points between genders.
#'The variable of points has a relatively close relationship with attitude, with correlation value 0.437.  
...
summary(model2)
#'In Model2, 'points' is the target and both 'attitude'and 'stra' are   the explanatory variables. The formula is  points=8.9729+3.4658*attitude+0.9137*stra.
#'We could expect an average points of 8.9729 for all gender subjects with no any points in attitude and strategis question. If stra remains constant, points will differ by 
#'3.4658 unit on average when attitude differed by 1 unit.Similarly, points will change 0.9137 unit when sta differed by one unit if attitude keeps constant.
#'The regression modle account for 20.48% of the variance. Since the points can be affected by other factors apart from attitude and sta, so the multiple R-squared value is lower
#'than 50%.  However, we have statistically a significant predictor in attitude, we can still use the formula for prediction. 
par(mfrow=c(2,2))
